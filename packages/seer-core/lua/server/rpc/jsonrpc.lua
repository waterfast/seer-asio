...
-- ============================ JSON-RPC 2.0 ============================
--
-- 本文件**整份抄自** freekill-core 的 `lua/server/rpc/jsonrpc.lua`（原作者注释见下方
-- 第一行），只做了三处改动：
--   1. cbor 的 require 路径改成 `cbor`（我们的第三方库放在 `lua/lib/`）；
--   2. 环境变量名 `FK_RPC_MODE` → `SEER_RPC_MODE`；
--   3. 补了这份说明。
--
-- 它实现的是 JSON-RPC 2.0 的**简化版**（用到啥实现啥）：
--   * Request/Response 带 `id`
--   * Notification 不带 `id`（不回复）
--   * 标准的 error code（-32700 起那套）
-- 序列化可以在 JSON 与 CBOR 之间切换（`SEER_RPC_MODE=cbor`），字段名会换成整数键——
-- 这就是 freekill 为了性能走过的路，我们先把口子留着。
--
-- 协议本身与"业务方法"无关：方法表由 `dispatchers.lua` 给出，
-- 所以换协议、换传输都不用动业务代码。
...
local RPC_MODE = os.getenv("SEER_RPC_MODE") == "cbor" and "cbor" or "json"
